...
void rxMode(unsigned char messageSize)
{
  DDRD |= DAT;
  mode = 1;
...
void txMode(unsigned char messageSize)
{
  DDRD |= DAT;
  mode = 0;
  if(messageSize) payloadSize = messageSize, configure();
...
void write(unsigned char* dataBuffer)
{
...
void read(unsigned char* dataBuffer)
{
  if(!dataBuffer) dataBuffer = (unsigned char*) data;
  DISABLE_CHIP;
  _delay_ms(2);
...
void loadConfiguration(bool modeSwitchOnly)
{
  DISABLE_CHIP;
  SELECT_CHIP;
  _delay_us(5);
